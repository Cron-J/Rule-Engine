...
export default class SimpleConditionComponent extends React.Component{
  ...
  renderRightExpression(instance) {
    if (instance && this.props.simplecondition.operator && this.props.simplecondition.operator.label) {
      this.rightExpressionComponent = (<ExpressionComponent schema={this.schema} className="expression" instanceType={this.instance} expression={this.props.simplecondition.rhexpression} expressionType="rhexpression" onPropertyChange={this.propertyChanged.bind(this)}/>);
    }else {
  ...
  render() {
    return (
      <div className="form-inline">
        <ExpressionComponent schema={this.schema} className="expression" expression={this.props.simplecondition.lhexpression} expressionType="lhexpression" onPropertyChange={this.propertyChanged.bind(this)}/>
  ...
  filteroperators() {
    ...
    let expression = this.props.simplecondition.lhexpression;
    if (expression.isVariable && this.schema && expression.variable) {
      this.instance = this.operatorType(expression.variable, this.schema);
      this.getOperatorlist(this.instance);
      this.renderRightExpression(this.instance);
    } else if (expression.isVariable === false && this.schema && expression.variable) {
